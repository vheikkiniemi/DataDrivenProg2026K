...
2.  **Start the app**:
    ...
    ```
3.  In the sidebar, upload your CSVs (you can select many at once).  
    Use the slider to set the **daily average temperature** range.

***

## What the app expects (and handles)

*   **CSV format** (matches your sample):
...
*   **Time zone:** offsets like `+02:00` in the `Time` column.  
    The app parses to UTC and safely **converts to Europe/Helsinki** before taking the date, so results don’t drift over midnight due to time zones or DST.

*   **Temperature column:**  
    Prefers `Daily average temperature`. If that’s not present, it will fall back to **any** column that contains the word `temperature`.

*   **Duplicate dates across files:**  
    If a day appears in multiple files, the app averages the duplicate daily values (they should be identical if it’s the daily average column repeated per hour).

***

## Features you’ll see

*   **Range slider** (°C) based on your data’s min/max.
*   **Matching days table** with:
    *   Date (e.g., `2025-01-03 (Fri)`)
    *   Daily average temperature (°C)
*   **Download button** to export matching days.
*   **Optional chart** to quickly visualize all daily temperatures.

***

## Code (for your reference)

If you prefer to copy-paste, here’s the complete `streamlit_app.py` content:
...
*   If your CSVs use a different encoding, the app tries `utf-8`, `utf-8-sig`, `cp1252`, and `latin-1` automatically.
*   If a **temperature column** is named something else (but includes the word “temperature”), it will still work.
*   If you want **exclusive** bounds instead of inclusive, I can tweak the filter to use `>` and `<`.

***

If you’d like, I can also add:

*   A **date range selector** (e.g., only 2018–2022).
*   A toggle for **inclusive/exclusive** bounds.
*   A quick **monthly distribution** summary for the matched days.

Shall I add any of those for your teaching/demo use?
